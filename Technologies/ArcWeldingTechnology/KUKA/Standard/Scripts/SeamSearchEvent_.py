...
def PostProcessAttributesUpload(Operator: CENPyOlpEvent_PEOperator):
   # get logger
   logging = Operator.GetLoggerOperator()
   # debug logging: create attributes
...
def PostCompute(Operator: CENPyOlpEvent_EventComputeOperator):
   ...
   #    logging.LogError("could not calculate collision point matrix for touch point event")
      ...
      downUnder = refTpElement.GetMatrixToActiveBaseFrame()
      if searchMoTypeIndex == LIN:
         refTpTag = Operator.MoveLin(downUnder)
      else:
         refTpTag = Operator.MovePTP(downUnder)

   refTpTag.GetGlobalTransformedMatrix().GetRotation(True)

   # tpMat = refTpTag.GetGlobalTransformedMatrixUnaligned()
   # refTpTag.GetMatrixToActiveBaseFrame().GetRotation(True)

   eventOperator.AddEvent(TOUCH_POINT_START_APP_EVENT_UUID, refTpElement, TPINSERTPOS_INSERTBEFORE)
   ...
   touchSpeedEvent=eventOperator.AddSpeed(refTpElement,TPINSERTPOS_INSERTBEFORE)
