...
def ModifyActiveProgram(Operator):
   ...
   # -------------------------------------
   CalculateNumberOfTouchesPerTouchId(weldingGroups, Operator)
   return
...
def CalculateNumberOfTouchesPerTouchId(WeldGroups, Operator):
   ...
   for weldGroup in WeldGroups:
      ...
      for touchId in listTouchIds:
         ...
         if any(listOfTouchOp):
            ...
            # Touch Counter "Touch_Cntr" Attribute is on Operation's-TPE
            for weldOperation in weldGroup.GetWeldingOperations():
               if (weldOperation != None):
                  SetTouchCntrAttribOnEvent(Operator, weldOperation, len(listOfTouchOp))

# HELPER - START
                  
# set on TOUCH_CONNECT_EVENT Touch_Cntr-integer attribute
def SetTouchCntrAttribOnEvent(Operator, Operation, number):
   eventHandler = Operator.GetEventHandler()
   opTpes = Operation.GetTpElements()
   # search for TOUCH_CONNECT_EVENT = "ConnectTouchProcessPointEvent" on TPEs
   for opTpe in opTpes:
      tpeHasConnectEvents = eventHandler.GetEventsByName(opTpe, TOUCH_CONNECT_EVENT)
      if any(tpeHasConnectEvents):
         for connectEvent in tpeHasConnectEvents:
            # set Attribute "Touch_Cntr" with No. of Touch-Ops in OpGroup
            connectEvent.SetInteger(AW_EVT_TOUCH_COUNTER, number)
