...
def CreateWeldingGroups(teachHandler, eventHandler, logging, program, weldingOps, touchOps):
   ...
   for weldingOp in weldingOps:
      ...
      if touchOperationFound == True or avoidTouchIdWithoutTouchOp == True:
         ...
         weldingGroupNr += grpNrIncrement
         if (gConnectionType == AW_CONNECT_START_END):
            grpNrIncrement = 2 # ConnectType::StartEnd needs upcount 2 here if more than one new Group
...
# --------------------------------------------------------------------------------------------
   ...
   avoidTouchIdWithoutTouchOp = GetProgramBoolAttribute(Program, AW_AVOID_TOUCHID_EVENT_WITHOUT_TOUCHOP)
   attTouchCnt = GetProgramIntegerAttribute(Program, AW_GLOBAL_TOUCH_COUNTER)
   attTouchCnt = (attTouchCnt - 1) if attTouchCnt is not None else 0
   grpAdd = 0
   for weldingGroup in WeldingGroups:
...
# --------------------------------------------------------------------------------------------
   ...
   # iterate though each welding group
   attTouchCnt = GetProgramIntegerAttribute(Program, AW_GLOBAL_TOUCH_COUNTER)
   attTouchCnt = attTouchCnt if attTouchCnt is not None else 0
   grpAdd = 0
   for weldingGroup in WeldingGroups:
...
# --------------------------------------------------------------------------------------------
   ...
   for weldingGroup in WeldingGroups:
      ...
      # ID counter for first/last process toolpath element
      if weldingGroup.GetWeldingGroupNumber() < attTouchCnt:
         counterFirstProcessTpe = attTouchCnt + grpAdd + weldingGroup.GetWeldingGroupNumber()  # 1st Hit
         counterLastProcessTpe = attTouchCnt + grpAdd + weldingGroup.GetWeldingGroupNumber()  # 1st Hit
      else:
         counterFirstProcessTpe = weldingGroup.GetWeldingGroupNumber()+ grpAdd  # already upcount in CreateWeldingGroups
         counterLastProcessTpe = weldingGroup.GetWeldingGroupNumber()+ grpAdd  # already upcount in CreateWeldingGroups
      #Logging.LogInfo('...................................counterFirstProcessTpe = ' + str(counterFirstProcessTpe) + ' from ' + str(weldingGroup.GetWeldingGroupNumber()))
      #Logging.LogInfo('...................................counterLastProcessTpe = ' + str(counterLastProcessTpe) + ' from ' + str(weldingGroup.GetWeldingGroupNumber()))
   ...
   def __init__(self, operation, eventHandler):
      ...
      if self.__workMethodName == WM_TOUCH:
         ...
         if gConnectionType == AW_CONNECT_SHORTEST_DISTANCE:
            ...
            if self.__tpeTouchOpCollisionPoint is None:
               raise ValueError(f"Couldn't find collision toolpath element while initialize TouchOperation")
         else:
      ...
      if self.__tpeTouchOpStartPoint is None:
         raise ValueError(f"Couldn't find start toolpath element while initialize TouchOperation")
