...
def PostInitAttributes(Operator: CENPyOlpEvent_AttribInitOperator):
   ...
   try:
      touch_id_attr = attribGetter.GetAttributeIntegerByName('TouchID')
   except Exception:
      # TouchID is used as deterministic counter for touch shift naming during download.
      touch_id_attr = attribCreator.AddInteger('TouchID', 1, 0, 9999, PROCESS_ATTRIBUTE | USER_ATTRIBUTE | OPERATION_ATTRIBUTE, 'TouchID')
   touch_id_attr.SetReComputeEnterState(ENTERSTATE_STARTWITHRULEEVENTS)
