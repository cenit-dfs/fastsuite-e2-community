...
def PostWmInitAttributes(Operator):
   ...
   touchMethod.SetOlpProperty(USER_ATTRIBUTE | OPERATION_ATTRIBUTE | OPERATION_GROUP_ATTRIBUTE | GLOBAL_ATTRIBUTE)
   touchMethod.SetVisibility(True)
   try:
   ...
   attribCreator.AddDouble(AW_TOUCHSENS_APPR_RETR_FLYBY, 0.005,0,0.1,0.001, GLOBAL_ATTRIBUTE | OPERATION_GROUP_ATTRIBUTE | OPERATION_ATTRIBUTE | PROCESS_ATTRIBUTE | USER_ATTRIBUTE, ATTRIB_LENGTH, AW_TOUCHSENS_APPR_RETR_FLYBY)
...
def UpdateAutoDefault(Operator):
   # get logger
   logging = Operator.GetLoggerOperator()
   # debug logging
   logging.LogDebug(FILE_NAME + DEBUG_POST_ON_ATTRIB_CHANGE_START)
